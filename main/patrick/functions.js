...
function block_hit(){
	//called when the player has jumped up into a block.
	block_hit_sound.play();
	game_world[Math.floor((player.y-1)/40)][Math.floor((player.x + 5 + current_offset_x)/40)].affect_hit();
	game_world[Math.floor((player.y-1)/40)][Math.floor((player.x + 25 + current_offset_x)/40)].affect_hit();
}
function render_player(){
	...
	ctx.scale(-1,1);
	//ctx.drawImage(player_left,player.x,player.y,40,40);
	
	}else{
		//facing right
	if (player.current_action=="walking"){
	ctx.drawImage(playerc,25*Math.floor(current),1,25,43,player.x,player.y,30,40);}else{
		ctx.drawImage(playerc,25*7,1,25,43,player.x,player.y,30,40);
	}
	//ctx.drawImage(player_right,player.x,player.y,40,40);
	}
}
